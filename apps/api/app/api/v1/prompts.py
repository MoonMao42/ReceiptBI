...
async def _get_prompt_or_404(db: AsyncSession, prompt_id: UUID) -> Prompt:
...
async def _clear_default_prompt(db: AsyncSession) -> None:
    result = await db.execute(select(Prompt).where(Prompt.is_default.is_(True)))
...
async def create_prompt(
    ...
    """创建提示词"""
    if data.is_default:
        await _clear_default_prompt(db)
...
async def update_prompt(
    ...
    """更新提示词（创建新版本）"""
    old_prompt = await _get_prompt_or_404(db, prompt_id)
...
async def get_prompt_versions(
    ...
    result = await db.execute(
        select(Prompt)
        .where((Prompt.id == root_id) | (Prompt.parent_id == root_id))
        .order_by(Prompt.version.desc())
    )
    versions = result.scalars().all()
    if prompt.id not in {item.id for item in versions}:
...
async def rollback_prompt(
    ...
    """回滚到指定版本"""
    current_prompt = await _get_prompt_or_404(db, prompt_id)
    root_id = await _find_root_prompt_id(db, current_prompt)
    result = await db.execute(
        select(Prompt).where(
            ((Prompt.id == root_id) | (Prompt.parent_id == root_id)) & (Prompt.version == version)
        )
    )
