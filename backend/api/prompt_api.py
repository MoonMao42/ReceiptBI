...
def _flatten_prompts(config):
    ...
    if 'SQL_ONLY' in system_messages:
        flat['sqlOnlyPrompt'] = system_messages['SQL_ONLY'].get('zh', '')
        flat['directSql'] = flat['sqlOnlyPrompt']
...
def save_prompts():
    ...
    try:
        ...
        if 'sqlOnlyPrompt' in data:
            new_config['systemMessage'].setdefault('SQL_ONLY', {})['zh'] = data['sqlOnlyPrompt']
            new_config['systemMessage']['SQL_ONLY'].setdefault('en', '')
        ...
        # 兼容旧的directSql/aiAnalysis字段
        if 'directSql' in data and not data.get('sqlOnlyPrompt'):
            new_config['systemMessage'].setdefault('SQL_ONLY', {})['zh'] = data['directSql']
            new_config['systemMessage']['SQL_ONLY'].setdefault('en', '')
