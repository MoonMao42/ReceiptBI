...
基于 Python 与 PydanticAI 构建，有以下特点：

- **语义隔离**：业务指标、维度与关联规则直接绑定在对应数据表下方。当不同表含有相同字段名时，仅在当前表的作用域内解析，避免同名字段含义混淆。
- **数据预检**：导入数据库时会自动检查字段数据类型与离群值，清洗规则单独保存，不修改原始数据文件。
- **业务规则确认**：遇到口径不明确的规则（如退款记录如何算入营收）时，暂停并向用户提出具体的单选项，由用户确认后再继续计算，同时会记入语义层。
- **沙箱隔离运行**：生成的 Python 计算与绘图代码在独立沙箱中执行。

### Rust SQLite ，二进制文件快且准

桌面端对 SQLite 的查询交由独立的 Rust 可执行程序（`sqlite-executor-core`）托管：

- **文件身份校验**：查询前后核对数据库文件的设备号、Inode、修改时间与文件大小。
- **查询计划预算审查**：运行 SQL 前解析 `EXPLAIN QUERY PLAN`。超过全表扫描上限、临时 B-Tree 预算或步数限制的查询会被拦截。
- **只读防御模式**：开启 SQLite 防御模式，禁用写操作、触发器与自定义扩展函数，仅允许安全的内置标量与聚合函数。

### 本地文件与数据库引擎

- **文件查询**：使用 DuckDB 处理本地 CSV、XLSX、Parquet 和 JSON 文件，任何随手的原始文件就能进行分析。
- **数据库支持**：提供 MySQL、PostgreSQL 和 SQLite 的支持。

### 语义层项目理解

- **可以提供LLM分析语义层**：高级model（类似gpt或claude前沿模型）分析一次语义层，存储后，可用弱model来进行查询
- **分层语义层**：在agent探索到具体某个表的时候，表下的语义层信息才会提供给model

### 报表处理

- **报表支持**：支持编辑报表或将其导出为pdf
- **支持ai自动整理报表格式**：ai负责排版运作。
